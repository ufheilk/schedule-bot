...
;; Asgn 2 HELPER
;; Takes in a map m and an unwanted key bad-k
...
(defn state-remove [m ks]
  (let [cur-key (first ks)
        same-lvl-elems (dissoc m cur-key)]
    (if (empty? (rest ks))
      same-lvl-elems
      (let [lower-dissoc (state-remove (cur-key m) (rest ks))]
        (if (empty? lower-dissoc)
          same-lvl-elems
          (assoc same-lvl-elems cur-key lower-dissoc))))))
